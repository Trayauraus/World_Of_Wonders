# ==============================================================================
# Player Controller for a 2D Platformer (Multiplayer Edition - Complete)
# ==============================================================================
...
@export var spawn_height_offset: float = -50.0 ## Spawns player X pixels higher so they fall
@export var sync_position_smoothness: float = 20.0 ## Higher = snappier, Lower = smoother interpolation

@export var color: Color = Color(1.0, 0.649, 0.747, 1.0)
@export var host_color: Color = Color(0.143, 0.6, 0.715, 1.0)
...
@export var idle_anim_count: int = 4 ## Amount of "Idle_X" animations you have.
...
@onready var dash_left_sound: AudioStreamPlayer = $SFX/DashRedLeft
@onready var dash_right_sound: AudioStreamPlayer = $SFX/DashRedRight
...
# This happens before _ready
func _enter_tree():
	# If this specific node is named "Player", it's the host.
	if name == "Player":
		set_multiplayer_authority(1)
	else:
		# Otherwise, use the numeric ID (for joined players)
		var id = name.to_int()
		if id != 0:
			set_multiplayer_authority(id)
		if id == 1:
			await get_tree().process_frame 
			animated_sprite.self_modulate = host_color
			animated_sprite.material.set_shader_parameter("flash_color", host_color)


func _ready():
	print_rich("[color=green]MultiPLAYER Script Activated")
	# var mp = get_tree().root.find_child("Multiplayer", true, false)
	# if mp: mp.notify_client_ready.rpc_id(1)
	
	# --- FIX 1: Add to Group ---
	# The carrot script specifically checks: if body.is_in_group("Player")
	add_to_group("Player") 

	if not is_multiplayer_authority():
		# Force particles off for anyone who isn't the "owner" of this player
	...
	if is_multiplayer_authority():
		# YOU are this player (Local Owner)
		if camera_2d:
			camera_2d.make_current()
		
		# Prevent spawning inside floor by applying offset
		global_position.y += spawn_height_offset
		
		if not has_light:
			if player_light: player_light.hide()
	else:
		# YOU ARE NOT this player (Client/Puppet)
		if camera_2d:
			camera_2d.enabled = false # Disable camera for other players
			$Hitbox.disabled = true #Disable collision for other players
		# Note: We do NOT disable collision, but we don't run physics process for them.
		animated_sprite.self_modulate = color
		animated_sprite.material.set_shader_parameter("flash_color", color)
		animated_sprite.material.set_shader_parameter("flash_modifier", 0.45)
		
		# --- FIX 2: Collision Logic ---
		# Only disable the hitbox if we are a CLIENT looking at ANOTHER PLAYER.
		# The SERVER needs the hitbox enabled to detect when this player touches a carrot.
	...
	# The rest of your code to find wind particles only runs for the LOCAL player
	var scene_root = get_owner()
	if scene_root and is_multiplayer_authority():
		...
		if not wind_parent_node:
			var level_container = scene_root.get_node_or_null("LevelContainer")
			if level_container:
				for child in level_container.get_children():
					if "The_Abyss" in child.name or child.name.match("???_*"): 
						var target_path = "Universal Scene/UNIVERSAL LV Nodes/Ash Follow Cam/WIND"
						wind_parent_node = child.get_node_or_null(target_path)
						break
		if wind_parent_node:
	...
	# Screen fade only for the local player
	if is_multiplayer_authority():
		if fade_on_start:
		...
		if screen_fader: screen_fader.hide() 
...
func _input(event: InputEvent) -> void:
	if not is_multiplayer_authority(): return # Only control our own player
...
func _physics_process(delta: float) -> void:
	...
	if not is_multiplayer_authority():
		# -- CLIENT/PUPPET LOGIC --
		# If we are not the authority, we do NOT calculate physics/inputs.
		# We rely on the MultiplayerSynchronizer to update position and variables.
		# We strictly run animation logic so the character looks right.
		_handle_animation()
		
		# Ensure synced sleeping states trigger the visual loops
		if _is_sleeping and animated_sprite.animation != "Sleep_Loop" and animated_sprite.animation != "Sleep_Begin":
			animated_sprite.play("Sleep_Loop")
			
		return # EXIT HERE for non-local players
		
	# =======================================================
	# AUTHORITY LOGIC (Physics & Input)
	# =======================================================
	
	if OS.is_debug_build():
		_handle_debug_input()

	# 1. Handle the "Dead" State
	if Global.is_dead and not _is_dev_mode:
		velocity.y = 200
		velocity.x = 0
		_handle_animation()
		move_and_slide()
		return # Exit early so we don't run movement logic below
	
	# 2. Safety check for Time Scale
...
func _handle_movement() -> void:
	...
	if direction != 0 and _is_camera_zoomed:
		_revert_idle_zoom()
...
func _handle_dash() -> void:
	...
	# SFX: Only play if we are the authority (Client sfx disabled per request)
	if dash_left_sound and dash_right_sound and is_multiplayer_authority():
...
##Handles ALL Player Animations (Runs on both Authority and Clients)
func _handle_animation() -> void:
	...
	# Since we are syncing velocity, we can use it for jump anims on clients too
...
func _on_animation_finished():
	# If we are not authority, we just play along with the state changes
	# but we generally do not trigger logic changes here unless it's pure visual cleanup.
...
func _perform_jump() -> void:
	...
	if jump_sound and is_multiplayer_authority():
		jump_sound.play()
...
# ==============================================================================
# Restored Functions (Ghost Trail & Flash)
# ==============================================================================
...
func _add_ghost():
	# If this is called on a client, it's fine because it's purely visual
	if not ghost_scene or not animated_sprite: 
		return
	
	# Since ghosts are unparented visual effects, we add them to the scene
